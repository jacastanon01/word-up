...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            model.wordSubmissions.forEach(function(word, index) {
                //if (model.wordSubmissions.hasOwnProperty(index)) {}
                if (!(model.wordSubmissions[index].hasOwnProperty("isRealWord"))) {
                    word.isRealWord = theAnswer;
                } 
            });
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        if (wordSubmission.isRealWord) {
            ...
            var scoreChip = $("<span></span>").text(" " + score)
                .attr("class", "tag-sm real-score-chip")
                
                
        } else {
            var scoreChip = $("<span></span>").text(" X")
                .attr("class", "tag-sm not-real-chip");
        }
        // TODO 17
